...
def draw_mona(pos, direction):
    ...
    screen.blit(mona.frame(frame), rect(pos[0], pos[1], 28 * direction, 24))
...
def draw_cursor(cursor):
    ...
    # note: the original used an XOR brush here (visible against any background);
    # the current API has no equivalent, so we pulse a plain grey instead
    i = (math.sin(badge.ticks / 250) * 127) + 127
