...
last_scan_tick = 0
last_connect_attempt = 0  # Timestamp (badge.ticks) of last explicit wlan.connect() attempt (post-scan)
...
def connect_wifi():
    ...
    # Still trying
    elapsed = (badge.ticks - ticks_start) // 1000
...
def draw_control_menu():
    ...
    screen.pen = gray
    screen.shape(shape.rectangle(5, 16, 150, 1))
...
def draw_color_picker():
    ...
    sw = color.rgb(r, g, b)
    screen.pen = sw
    screen.shape(shape.rectangle(50, 45, 60, 30))
    screen.pen = white
    screen.shape(shape.rectangle(49, 44, 62, 1))
    screen.shape(shape.rectangle(49, 75, 62, 1))
    screen.shape(shape.rectangle(49, 44, 1, 32))
    screen.shape(shape.rectangle(110, 44, 1, 32))
...
def draw_brightness_picker():
    ...
    screen.pen = phosphor
    screen.shape(shape.rectangle(bar_x, bar_y, filled_w, bar_h))
    
    # Border
    screen.pen = white
    screen.shape(shape.rectangle(bar_x - 1, bar_y - 1, bar_w + 2, 1))
    screen.shape(shape.rectangle(bar_x - 1, bar_y + bar_h, bar_w + 2, 1))
    screen.shape(shape.rectangle(bar_x - 1, bar_y - 1, 1, bar_h + 2))
    screen.shape(shape.rectangle(bar_x + bar_w, bar_y - 1, 1, bar_h + 2))
...
def draw_ui():
    ...
    if wifi_connected and state_checked and wled_connected:
        ...
        screen.pen = green if wled_power else red
        ...
        if wled_power:
            if wled_color:
                ...
                # Swatch
                sw = color.rgb(wled_color[0], wled_color[1], wled_color[2])
                screen.pen = sw
                screen.shape(shape.rectangle(60, 79, 40, 15))
                screen.pen = white
                screen.shape(shape.rectangle(59, 78, 42, 1))
                screen.shape(shape.rectangle(59, 94, 42, 1))
                screen.shape(shape.rectangle(59, 78, 1, 17))
                screen.shape(shape.rectangle(100, 78, 1, 17))
            else:
        ...
        if not wifi_connected:
            ...
            if ticks_start:
                center_text(f"t={(badge.ticks - ticks_start)//1000}s", 63)
...
def update():
    ...
    if badge.pressed(BUTTON_A) and wifi_connected and wled_connected:
        control_mode = True
        control_selection = 0
    ...
    # periodically persist UI selection (the current firmware's run() has no
    # on_exit hook, so we can't save only when leaving the app - see init())
    global _last_save_ticks
    if now - _last_save_ticks > 5000:
        _last_save_ticks = now
        State.save("wled", {
...
# note: the current firmware's run() only takes an update function - it
# has no init/on_exit hooks, so the periodic save inside update() above
# replaces the old on_exit() callback.
init()
run(update)