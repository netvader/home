...
WALL_COLOR = color.rgb(73, 219, 255)
DIVIDER_H_COLOR = color.rgb(255, 255, 0)
DIVIDER_V_COLOR = color.rgb(0, 255, 0)
BALL_COLORS = [
    color.rgb(255, 50, 50),
...
CLAIMED_COLOR = color.rgb(40, 40, 80, 200)
CURSOR_COLOR = color.rgb(255, 255, 255, 150)
...
class Ball:
    ...
    def draw(self):
        """Draw the ball"""
        screen.pen = BALL_COLORS[self.color_index % len(BALL_COLORS)]
        screen.shape(shape.circle(int(self.x), int(self.y), self.radius))
...
class Divider:
    ...
    def draw(self):
        ...
        if self.direction == "horizontal":
            ...
            screen.shape(shape.line(x1, int(self.y), x2, int(self.y), DIVIDER_THICKNESS))
        else:
            y1 = int(self.y - self.top_length)
            y2 = int(self.y + self.bottom_length)
            screen.shape(shape.line(int(self.x), y1, int(self.x), y2, DIVIDER_THICKNESS))
...
def update():
    ...
    if state["ready"]:
        ...
        if badge.pressed(BUTTON_B) and not state["divider_active"] and not state["won_level"]:
        ...
        if badge.pressed(BUTTON_B) and state["won_level"]:
            # Start next level
            start_level()
    ...
    if not state["game_over"] and not state["won_level"]:
        dt = min(badge.ticks_delta / 10, 5)  # Cap delta for stability
    ...
    screen.pen = CLAIMED_COLOR
    screen.shape(shape.rectangle(PLAY_AREA_X, PLAY_AREA_Y, PLAY_AREA_WIDTH, PLAY_AREA_HEIGHT))
    ...
    for area in state["claimed_areas"]:
        ...
        if has_ball:
            screen.pen = BACKGROUND
            screen.shape(shape.rectangle(int(area["x"]), int(area["y"]), int(area["width"]), int(area["height"])))
    
    # Draw play area border
    screen.pen = WALL_COLOR
    screen.shape(shape.rectangle(PLAY_AREA_X - 2, PLAY_AREA_Y - 2, PLAY_AREA_WIDTH + 4, PLAY_AREA_HEIGHT + 4).stroke(2))
    ...
    if not state["ready"] and not state["divider_active"] and not state["game_over"] and not state["won_level"]:
        ...
        if state["divider_direction"] == "horizontal":
            ...
            screen.shape(shape.line(PLAY_AREA_X, cursor_y, PLAY_AREA_X + PLAY_AREA_WIDTH, cursor_y, 1))
            # Draw crosshair
            screen.shape(shape.line(cursor_x - 3, cursor_y, cursor_x + 3, cursor_y, 2))
            screen.shape(shape.line(cursor_x, cursor_y - 3, cursor_x, cursor_y + 3, 2))
        else:
            # Show vertical line indicator
            screen.shape(shape.line(cursor_x, PLAY_AREA_Y, cursor_x, PLAY_AREA_Y + PLAY_AREA_HEIGHT, 1))
            # Draw crosshair
            screen.shape(shape.line(cursor_x - 3, cursor_y, cursor_x + 3, cursor_y, 2))
            screen.shape(shape.line(cursor_x, cursor_y - 3, cursor_x, cursor_y + 3, 2))
