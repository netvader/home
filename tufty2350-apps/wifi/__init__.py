...
def check_connection_status():
    ...
    if not connecting and WIFI_SSID and WIFI_PASSWORD:
        ...
        if last_attempt_time:
            elapsed_since_last = (badge.ticks - last_attempt_time) / 1000
        ...
        connecting = True
        connection_start_time = badge.ticks
    ...
    if connecting:
        elapsed = (badge.ticks - connection_start_time) / 1000
...
def update():
    ...
    try:
        ...
        if abs(scroll_delta) > 0.1:
            movement = scroll_delta * (badge.ticks_delta / 50)  # Faster animation
    ...
    screen.pen = gray
    screen.shape(shape.rectangle(5, 13, 150, 1))
    ...
    if not has_credentials:
        ...
        if scroll_y > 0:
            ...
            screen.shape(shape.rectangle(150, 15, 10, 2))  # Up indicator
        if scroll_y < max_scroll:  # Show down indicator if more content below
            screen.pen = color.rgb(255, 255, 255, 128)
            screen.shape(shape.rectangle(150, 103, 10, 2))  # Down indicator
    ...
    screen.font = small_font
    screen.pen = gray
    screen.shape(shape.rectangle(5, 103, 150, 1))
