...
# has no init/on_exit hooks (leaving the app via HOME resets the module on
# next launch rather than calling back into it). So instead of a deferred
# on_exit save, state is saved immediately at each point above where a
# stat changes, and init() is called once here before the loop starts.
init()
run(update)
