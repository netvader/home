import math
from badgeware_compat import SpriteSheet
...
def background(mona):
    ...
    screen.shape(shape.rectangle(0, 0, 160, floor_y))
    ...
    for y in range(8):
        for x in range(19):
            if (x + y) % 2 == 0:
                xo = math.sin(badge.ticks / 1000) * 2
                yo = math.cos(badge.ticks / 1000) * 2
    ...
    screen.shape(shape.line(px + 2, 20 + 2, px + 20, 15, 1))
    screen.shape(shape.line(px + 35 + 2, 20 + 2, px + 20, 15, 1))
    screen.pen = color.rgb(30, 40, 50, 100)
    screen.shape(shape.rectangle(px + 1, 20 + 1, 38, 28))
    screen.pen = color.rgb(50, 40, 30, 255)
    screen.shape(shape.rectangle(px, 20, 38, 28))
    screen.pen = color.rgb(120, 130, 140, 255)
    screen.shape(shape.rectangle(px + 2, 20 + 2, 38 - 4, 28 - 4))
    ...
    screen.shape(shape.rectangle(0, floor_y - 5, 160, 5))
    screen.shape(shape.rectangle(0, floor_y - 4, 160, 1))
...
def draw_button(x, y, label, active):
    ...
    screen.alpha = 255 if active else 150
