...
class ScrollList:
    ...
    def handle_io(self):
        self.connect_input(BUTTON_UP, self.on_button_up)
        self.connect_input(BUTTON_DOWN, self.on_button_down)
        self.connect_input(BUTTON_A, self.on_button_back)
        self.connect_input(BUTTON_C, self.on_button_forward)
    ...
    def render_item(self, item, index, brush=color.rgb(255, 255, 255)):
        is_selected = index == self.index

        if is_selected:
            screen.pen = color.rgb(0, 50, 0)
            screen.shape(shape.rectangle(0, self._current_render_y, SCREEN_WIDTH, 12))
