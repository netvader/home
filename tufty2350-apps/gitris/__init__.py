...
def _getrandbits(n):
    # Prefer MicroPython's getrandbits; fall back to simple LCG seeded by badge.ticks.
    try:
        return urandom.getrandbits(n)
    except AttributeError:
        # Very small fallback PRNG (LCG)
        s = _rng_state.get("s", (badge.ticks & 0x7fffffff) ^ 0xB5297A4D)
...
_rng_state = {"s": (badge.ticks & 0x7fffffff) ^ 0x1f123bb5}
...
# ---------------- GitHub-dark palette ----------------
COL_BG    = color.rgb(13, 17, 23)       # page bg
COL_GRID  = color.rgb(48, 54, 61)       # frame
COL_TEXT  = color.rgb(240, 246, 252)    # primary text
COL_DIM   = color.rgb(139, 148, 158, 230)

# Piece colors (Primer-ish accents)
COLORS = [
    color.rgb( 61, 199,  98),  # I  green
    color.rgb(250, 219,  94),  # O  yellow
    color.rgb(191, 128, 255),  # T  purple
    color.rgb( 70, 212, 143),  # S  emerald
    color.rgb(255, 121, 135),  # Z  coral/red
    color.rgb( 90, 150, 255),  # J  blue
    color.rgb(255, 170, 108),  # L  orange
]
CONFLICT_COLOR = color.rgb(87, 96, 106) # conflict/garbage row blocks
COL_GHOST = color.rgb(240, 246, 252, 70)
...
def _to_game():  state["screen"] = "play";  state["last_fall"] = badge.ticks
...
_toast_msg = {"t":0, "txt":""}
def _toast(s):
    _toast_msg["t"] = badge.ticks
    _toast_msg["txt"] = s

def _draw_toast():
    if not _toast_msg["txt"]:
        return
    if badge.ticks - _toast_msg["t"] > 1400:
        _toast_msg["txt"] = ""
        return
    screen.pen = color.rgb(0,0,0,160)
    screen.shape(shape.rectangle(PF_X, PF_Y+38, GRID_W*CELL, 14))
    screen.pen = COL_TEXT
    screen.text(_toast_msg["txt"], PF_X+6, PF_Y+40)

# ---------------- Input helpers ----------------
def _pressed(btn): return badge.pressed(btn)
def _pause_pressed():
    return _pressed(BUTTON_HOME) or (_pressed(BUTTON_B) and _pressed(BUTTON_C))

# ---------------- Input handlers ----------------
def _handle_title():
    # Any of these starts a new run
    if _pressed(BUTTON_A) or _pressed(BUTTON_B) or _pressed(BUTTON_C) or _pressed(BUTTON_DOWN) or _pressed(BUTTON_UP):
        _reset_run(); _to_game()

def _handle_pause():
    # A = resume, B = title
    if _pressed(BUTTON_A): _to_game()
    elif _pressed(BUTTON_B): _to_title()

def _handle_gameover():
    if _pressed(BUTTON_A) or _pressed(BUTTON_B) or _pressed(BUTTON_C):
        _to_title()
...
def _handle_play():
    ...
    # Rotation
    if _pressed(BUTTON_UP): _rotate_cw()

    # Movement (A = left, B = right) with DAS/ARR
    if _pressed(BUTTON_A):
    ...
    # Soft drop
    if _pressed(BUTTON_DOWN):
        if _move(0,1): state["score"] += 1
        state["last_fall"] = now

    # Hard drop
    if _pressed(BUTTON_C):
...
def _draw_cell(px, py, color, alpha_outline=False):
    screen.pen = color
    screen.shape(shape.rectangle(px, py, CELL, CELL))
    if alpha_outline:
        screen.pen = COL_LOCK
        screen.shape(shape.rectangle(px, py, CELL, CELL))

def _draw_grid():
    screen.pen = COL_BG; screen.clear()
    screen.pen = COL_GRID
    screen.shape(shape.rectangle(PF_X-1, PF_Y-1, GRID_W*CELL+2, GRID_H*CELL+2))
...
def _draw_sidebar():
    ...
    if nx is not None:
        ...
        for cx, cy in SHAPES[nx][0]:
            ...
            screen.pen = COLORS[nx]
            screen.shape(shape.rectangle(px, py, 3, 3))

def _draw_title():
    _draw_grid(); _draw_board()
    screen.pen = COL_DIM;  screen.text("GITRIS", PF_X + 6, 10)
    screen.pen = COL_TEXT
