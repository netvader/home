...
class Brick:
    ...
    def draw(self):
        if self.alive:
            screen.pen = color.rgb(*self.color)
            screen.shape(shape.rectangle(self.x, self.y, BRICK_WIDTH, BRICK_HEIGHT))
...
class Paddle:
    ...
    def update(self, ball=None, auto_play=False, bricks=None):
        ...
        manual_input = badge.held(BUTTON_A) or badge.held(BUTTON_C)
    ...
    def draw(self):
        screen.pen = color.rgb(*PADDLE_COLOR)
        for i in range(PADDLE_SEGMENTS):
            x = self.x + (i * UNIT)
            screen.shape(shape.rectangle(x, self.y, SQUARE_SIZE, SQUARE_SIZE))
...
class Ball:
    ...
    def draw(self):
        screen.pen = color.rgb(*BALL_COLOR)
        screen.shape(shape.rectangle(int(self.x), int(self.y), BALL_SIZE, BALL_SIZE))
...
def update():
    ...
    # Clear screen
    screen.pen = color.rgb(*BACKGROUND_COLOR)
    screen.shape(shape.rectangle(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT))
