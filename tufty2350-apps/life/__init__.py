        ...
        for y in range(GRID_HEIGHT):
            ...
            for x in range(GRID_WIDTH):
                if self.grid[y][x]:
                    ...
                    screen.pen = NEIGHBOR_BRUSHES[neighbors]
                    cell_rect.transform = mat3().translate(x * GRID_SIZE, py)
                    screen.shape(cell_rect)
...
def update():
    ...
    if badge.pressed(BUTTON_B):
        ...
        info_timer = badge.ticks + 1000  # Show "Regenerated" for 1 second    
    if badge.pressed(BUTTON_C):
        ...
        info_timer = badge.ticks + 1000  # Show palette name for 1 second
    # Check for auto-regeneration every 10 minutes
    if badge.ticks - game.last_regen > game.regen_interval:
        game.randomize()
        # Don't show message for automatic regeneration
    
    # Update game logic
    if badge.ticks - game.last_update > game.update_interval:
        game.last_update = badge.ticks
        game.update()
    ...
    # Show info message
    if show_info and badge.ticks < info_timer:
        w, _ = screen.measure_text(info_message)
        # Draw background for text
        screen.pen = INFO_BG_BRUSH
        screen.shape(shape.rectangle(80 - (w // 2) - 2, 55, w + 4, 10))
        # Draw text
        screen.pen = TEXT_BRUSH
        screen.text(info_message, 80 - (w // 2), 56)
    elif badge.ticks >= info_timer:
        show_info = False
